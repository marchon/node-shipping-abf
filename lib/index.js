  ...
  $scope.buildAPIMethod = function(resource) {
    return function(params) {
      ...
      request.get(host + resource.path, {qs: params}, function(err, response, body) {
        ...
        parser.parseString(body, {explicitArray: false}, function(err, json) {
          if(err) {
            return defer.reject(err);
          }

          if(json.ABF.ERRORS) {
            return defer.reject({code: json.ABF.ERRORS.ERRORCODE, message: json.ABF.ERRORS.ERRORMESSAGE});
          } else if(json.ABF.ERROR) {
            return defer.reject({code: json.ABF.ERROR.ERRORCODE, message: json.ABF.ERROR.ERRORMESSAGE});
          }
